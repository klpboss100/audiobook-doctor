...
─────────────────────────────────────────
"한량의원 허윤의 팔도유람기" 전용 오디오북 제작기
(audiobook-maker에서 포크, 이 책 전용으로 고정)
"""
...
# 페이지 설정 & CSS
# ══════════════════════════════════════════
st.set_page_config(page_title="허윤 오디오북", page_icon="🎧", layout="wide")
...
with st.sidebar:
    ...
    # ── 소설 설정 ────────────────────────
    st.markdown("<div style='background:linear-gradient(90deg,#92400e,#f59e0b);border-radius:8px 8px 0 0;padding:8px 12px;margin-top:6px'><span style='color:white;font-size:13px;font-weight:800;cursor:help' title='시대배경+문체 설정 | 품질검사 정확도 향상'>📖 소설 설정</span></div><div style='border:2px solid #92400e;border-top:none;border-radius:0 0 8px 8px;padding:8px 10px;margin-bottom:6px'>", unsafe_allow_html=True)
    novel_era = st.text_input("시대 배경", value=cfg.get("novel_era","조선시대"),
                               placeholder="예: 조선시대",
                               key="novel_era",
                               help="구체적일수록 품질검사 정확도↑")
    if novel_era != cfg.get("novel_era","조선시대") and not IS_CLOUD:
        cfg["novel_era"] = novel_era
        save_config(cfg)
    novel_style_list = st.multiselect("문체 스타일",
        ["표준 현대어","고어/사극체","방언 포함","대화체"],
        default=["고어/사극체"], key="novel_style",
        help="복수 선택 가능\n표준 현대어: 일반 소설\n고어/사극체: 하오체 허용\n방언 포함: 사투리 허용\n대화체: 말하듯 쓴 글")
    novel_style = ", ".join(novel_style_list) if novel_style_list else "고어/사극체"
    st.markdown("</div></div>", unsafe_allow_html=True)
    ...
    <div style='margin-bottom:6px;display:grid;grid-template-columns:80px 1fr;align-items:center;gap:14px;max-width:500px'>
      ...
      <div>
        <div style='line-height:1.1'>
          <span style='font-size:32px;font-weight:800;color:#7c3aed'>허윤</span>
          <span style='font-size:32px;font-weight:300;color:#1a1a2e'>오디오북</span>
        </div>
        <div style='font-size:13px;color:#9f7aea;font-weight:500;margin-top:4px'>
          한량의원 허윤의 팔도유람기 · 전용 스튜디오
        </div>
      </div>
    </div>
    """, unsafe_allow_html=True)
...
chapter_name = first_line_title(manuscript)
st.caption(f"📌 챕터명 (원고 첫 줄 자동 사용): **{chapter_name or '(원고를 입력하면 자동으로 채워집니다)'}**")
...
# STEP 3: 오디오 태그 원고
# ══════════════════════════════════════════
if 'tagged_script' not in st.session_state:
    with st.expander("📋 이미 태그 작업된 원고가 있나요? — 1·2단계 건너뛰고 여기서 3단계 바로 시작"):
